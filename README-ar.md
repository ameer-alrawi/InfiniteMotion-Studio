<div align="center">
  ...
  **محرر فيديو مكاني (Spatial) احترافي ومغلق المصدر، يتميز بمساحة عمل لا نهائية، كاميرا افتراضية، تصيير عتادي حتمي (Deterministic Hardware Rendering)، وإمكانيات متقدمة في تصميم الموشن جرافيك.**
...
مرحباً بك في **InfiniteMotion Studio**، بيئة الجيل القادم لتحرير الفيديو مباشرة من المتصفح. بُنيت هذه المنصة كبرمجية احترافية مغلقة المصدر لتتجاوز قيود مساحات العمل التقليدية وتقدم تجربة تحرير مكانية (Spatial Editing). يمكن للمستخدمين تحريك العناصر ديناميكياً، التحكم بكاميرا افتراضية للتنقل داخل المشاهد المعقدة، وحقن أكواد HTML/CSS برمجية—كل ذلك يتم تصييره وتصديره محلياً عبر مسار (Pipeline) مخصص يعتمد على تسريع العتاد باستخدام WebCodecs.

![بنية النظام والابتكارات التقنية](src/screenshot1.png)

## 🏗️ بنية النظام والابتكارات التقنية

نظراً لأن InfiniteMotion Studio يمثل قاعدة برمجية مغلقة المصدر، يقدم هذا القسم نظرة عامة على القرارات الهندسية والمعمارية التي تشغل المنصة.

### 1. محرك تصيير Canvas مخصص
يرتكز الاستوديو في جوهره على مسار تصيير (Rendering Pipeline) عالي الكفاءة مبني على HTML5 Canvas.
*   **رياضيات المصفوفات والتحويلات (Matrix Math):** تتم معالجة التموضع المكاني، التدوير، والتحجيم عبر أدوات مخصصة لرياضيات المصفوفات، مما يتيح لك التحريك والتقريب اللانهائي دون أي فقدان في الدقة.
*   **خلفيات ديناميكية:** يولد المحرك خلفيات معقدة برمجياً في الوقت الفعلي، بما في ذلك تدرجات شبكية متحركة (Mesh Gradients)، تأثيرات الزجاج المغشى (Frosted Glass)، وأنماط خطوط الشاشة الكلاسيكية (CRT Scanlines).
*   **دمج هجين بين DOM و Canvas:** بينما تُرسَم الأصول القياسية (فيديو، صور، نصوص) مباشرة على الـ Canvas، يتم تصيير الأصول البرمجية (HTML/CSS/SVG) عبر طبقة DOM متزامنة تتطابق تماماً مع تحويلات الكاميرا الافتراضية، مما يسمح بدمج حركات CSS بسلاسة تامة داخل مساحة الفيديو.

### 2. التصدير الحتمي ومزامنة الوسائط

![التصدير الحتمي ومزامنة الوسائط](src/screenshot2.png)

يتطلب تصدير فيديو عالي الجودة من المتصفح تجاوز القيود التقليدية للتشغيل في الوقت الفعلي.
*   **تكامل WebCodecs:** يعتمد مسار التصدير على واجهات `VideoEncoder` و `AudioEncoder` منخفضة المستوى لخبز (Bake) الإطارات مباشرة إلى حاويات MP4 (H.264/AAC) أو WebM (VP9/Opus).
*   **مزامنة الإطارات بقفل ثنائي (Binary-Lock):** لضمان عدم سقوط أي إطار أثناء التصيير، يستخدم المحرك آلية قفل مخصصة توقف حلقة التصيير (Render Loop) مؤقتاً حتى تؤكد عناصر `HTMLVideoElement` أن حالة `readyState` قد قامت بتخزين الإطار المطلوب بدقة.
*   **المكساج عبر OfflineAudioContext:** لا يتم مجرد تسجيل المسارات الصوتية؛ بل تُدمج وتُوزن وتُصيّر بالكامل داخل ذاكرة (Float32) باستخدام `OfflineAudioContext` من واجهة Web Audio API قبل دمجها (Multiplexing) في حاوية الفيديو النهائية.
